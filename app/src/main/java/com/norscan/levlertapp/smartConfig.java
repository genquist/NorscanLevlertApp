...
import com.norscan.levlertapp.utils.CC3XConstants;
import com.norscan.levlertapp.utils.CC3XDialogManager;
import com.norscan.levlertapp.utils.CC3XWifiManager;
import com.norscan.levlertapp.utils.CCX300Utils;

import java.util.Timer;
import java.util.TimerTask;

public class smartConfig extends Fragment implements View.OnClickListener,
        FirstTimeConfigListener, CompoundButton.OnCheckedChangeListener, TextWatcher
{
    ...
    public CC3XWifiManager getWiFiManagerInstance() {
        ...
        {
            mCC3XWifiManager = new CC3XWifiManager(getActivity());
        }
    ...
    private boolean checkNetwork(String str) {
        ...
        if (!(getWiFiManagerInstance().isWifiConnected()))
        {
            ...
            mCC3xDialogManager = new CC3XDialogManager(getActivity());
            getActivity().showDialog(NO_NETWORK_DIALOG_ID);
            return false;
    ...
    {
        mSendDataPackets = (Button) getActivity().findViewById(R.id.config_start_button);
        footerView = (RelativeLayout) getActivity().findViewById(R.id.config_footerview);
        mSSIDInputField = (EditText) getActivity().findViewById(R.id.config_ssid_input);
        mPasswordInputField = (EditText) getActivity().findViewById(R.id.config_passwd_input);
        mGateWayIPInputField = (TextView) getActivity().findViewById(R.id.config_gateway_input);
        mKeyInputField = (EditText) getActivity().findViewById(R.id.config_key_input);
        mDeviceNameInputField = (EditText) getActivity().findViewById(R.id.config_device_name_input);
        mconfigProgress = (ProgressBar) getActivity().findViewById(R.id.config_progress);
        mconfig_key_checkbox = (CheckBox) getActivity().findViewById(R.id.config_key_checkbox);
    ...
    private void sendPacketData() throws Exception
    {
        if (!isCalled)
        {
            ...
            try
            {
                firstConfig = getFirstTimeConfigInstance(smartConfig.this);
            } catch (Exception e)
            {
    ...
    public void onConfigurationChanged(Configuration newConfig)
    {
        ...
        if (!(CCX300Utils.isScreenXLarge(getActivity())))
        {
            return;
        }
    }
